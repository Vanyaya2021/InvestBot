...
from Handlers.CommandHandlers.BaseCommandHandler import menu,menuCommandHandler
from Handlers.CommandHandlers.TickerCommandHandler import tickerCommandHandler
from Handlers.CallBackQueryHandlers.AssetsCallbackQueryHandler import nextPageOfAssetsCallbackQueryHandler,backPageOfAssetsCallbackQueryHandler
from Handlers.CallBackQueryHandlers.FavoriteAssetsCallbackQueryHandler import nextPageOfFavoriteAssetsCallbackQueryHandler,backPageOfFavoriteAssetsCallbackQueryHandler
from Keyboards.OfficeKeyboards import sectionsMarkup
from Keyboards.General import AssetsPaginationKeyboard,FavoriteAssetsPaginationKeyboard
from Keyboards.Inline.PushNotificationsKeyboards import getPushNotificationKeyboard
from Handlers.CallBackQueryHandlers.pushNotificationManagerCQHandler import pushPriceCallbackQueryHandler,pushNewsCallbackQueryHandler
...
from DB.Queries.userQuery import getUserAssets,getUserByChatId,getUserFavoriteAssets
from Configurations.config import CHOOSE_OFFICE_SECTION,CHOOSEN_MY_ASSETS,MANAGE_PUSH,CHOOSEN_FAVORITE_ASSETS
...
def officeConversationHandler() -> ConversationHandler:
    handler = ConversationHandler(
        ...
        states={
            ...
            CHOOSEN_MY_ASSETS:[nextPageOfAssetsCallbackQueryHandler(),
                               backPageOfAssetsCallbackQueryHandler(),
                               MessageHandler(Filters.regex('^(В меню)$'), toMenu),
                               MessageHandler(Filters.regex('^(Избранное️ 🌟)$'), myAssetsFav),
                               MessageHandler(Filters.regex('^(Настройка пуш-уведомлений 🛎)$'), myPushNotifications),],
            CHOOSEN_FAVORITE_ASSETS:[nextPageOfFavoriteAssetsCallbackQueryHandler(),
                                     backPageOfFavoriteAssetsCallbackQueryHandler(),
                                     MessageHandler(Filters.regex('^(Мой портфель 💳)$'), myAssets),
                                     MessageHandler(Filters.regex('^(Настройка пуш-уведомлений 🛎)$'), myPushNotifications),
                                     menuCommandHandler(),
                                     MessageHandler(Filters.regex('^(В меню)$'), toMenu)],
            MANAGE_PUSH:[pushPriceCallbackQueryHandler(),
                         pushNewsCallbackQueryHandler(),
                         MessageHandler(Filters.regex('^(Мой портфель 💳)$'), myAssets),
                         MessageHandler(Filters.regex('^(Избранное️ 🌟)$'), myAssetsFav),
                         MessageHandler(Filters.regex('^(В меню)$'), toMenu),
                         menuCommandHandler()]
...
def myAssets(bot,update) -> int:
    ...
    bot.message.reply_text(generalMessage + message, parse_mode = "Markdown",
                           reply_markup = AssetsPaginationKeyboard(page=1, elements_on_page=elementsOnPage, all_pages=allPages))
...
def myAssetsFav(bot,update) -> int:
    chatId = bot.message.chat.id
    userAssets, allPages = getUserFavoriteAssets(chatId, page=1)
    elementsOnPage = len(userAssets)

    message = assetsMessage(userAssets)

    bot.message.reply_text(message, parse_mode="Markdown",
                           reply_markup=FavoriteAssetsPaginationKeyboard(page=1, elements_on_page=elementsOnPage,
                                                                 all_pages=allPages))
